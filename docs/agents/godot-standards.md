...
- **自适应期望公式（如 `floor(duration/interval)+1`）对任何 duration 都自洽，没有判别力**——节奏 / 时长类断言必须钉死黄金值（无放大路径的期望值做注释），自适应式只配当对照（实测：#35 cadence 断言在一场 lose 战斗上假绿，floor(30/9)+1 = 4 恰等于实际 4 hits）。
