...
- 找**MVP 施工规格** → GitHub issue #24「规格：MVP 核心循环垂直切片 + 内容管线」（/to-spec 对本目录全部文档的收拢产物，施工票从它出发）
...
- MVP 规格 = GitHub issue **#24**（本目录 12 份文档折叠后的施工计划）：唯一受测接缝 = 引擎侧会话门面公共 API；两条工作流（引擎侧 / 内容管线侧）在内容数据与校验器处交汇。本目录任何文档修订后，须回头核对 #24 对应条款是否失效。
